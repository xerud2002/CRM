...
export class DashboardService {
  ...
  async getContactMetrics(filter: DashboardFilterDto): Promise<ContactMetrics> {
    ...
    return {
      ...
      responded,
      respondedPercent:
        contacted > 0 ? Math.round((responded / contacted) * 1000) / 10 : 0,
      noResponse,
      noResponsePercent:
        contacted > 0 ? Math.round((noResponse / contacted) * 1000) / 10 : 0,
    };
  }
  ...
  async getMethodBreakdown(
    ...
    // Apply date filters
    if (filter.dateFrom) {
      emailQuery.andWhere('email.createdAt >= :dateFrom', {
        dateFrom: filter.dateFrom,
      });
      callQuery.andWhere('call.createdAt >= :dateFrom', {
        dateFrom: filter.dateFrom,
      });
    }
    if (filter.dateTo) {
      emailQuery.andWhere('email.createdAt <= :dateTo', {
        dateTo: filter.dateTo,
      });
      callQuery.andWhere('call.createdAt <= :dateTo', {
        dateTo: filter.dateTo,
      });
    }
    ...
    return {
      ...
      emailResponseRate:
        emailsSent > 0
          ? Math.round((emailsResponded / emailsSent) * 1000) / 10
          : 0,
      callsMade,
      callsAnswered,
      callAnswerRate:
        callsMade > 0 ? Math.round((callsAnswered / callsMade) * 1000) / 10 : 0,
    };
  }
  ...
  async getConversionFunnel(
    ...
    return {
      ...
      surveyBookedPercent:
        totalLeads > 0
          ? Math.round((surveyBooked / totalLeads) * 1000) / 10
          : 0,
      surveyCompleted,
      surveyCompletedPercent:
        surveyBooked > 0
          ? Math.round((surveyCompleted / surveyBooked) * 1000) / 10
          : 0,
      quoteSent,
      quoteSentPercent:
        surveyCompleted > 0
          ? Math.round((quoteSent / surveyCompleted) * 1000) / 10
          : 0,
      quoteAccepted,
      quoteAcceptedPercent:
        quoteSent > 0 ? Math.round((quoteAccepted / quoteSent) * 1000) / 10 : 0,
    };
  }
